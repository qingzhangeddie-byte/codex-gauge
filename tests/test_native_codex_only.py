...
class NativeCodexOnlyTests(unittest.TestCase):
    ...
    def test_native_app_draws_codex_usage_in_native_status_view(self):
        ...
        self.assertIn("let cornerRadius = min(1.4, rect.height / 3.0)", source)
        self.assertIn("let innerRect = rect.insetBy(dx: 1.0, dy: 1.0)", source)
        self.assertIn("NSColor(calibratedRed: 0.43, green: 0.49, blue: 0.63, alpha: 0.94)", source)
        self.assertIn("NSColor(calibratedWhite: 0.08, alpha: 0.88)", source)
