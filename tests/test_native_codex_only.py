...
class NativeCodexOnlyTests(unittest.TestCase):
    ...
    def test_native_app_draws_codex_usage_in_native_status_view(self):
        ...
        self.assertIn("private struct QuotaWindowStatus", source)
        self.assertIn("let quotaWindows: [QuotaWindowStatus]", source)
        self.assertIn("availableQuotaWindows(status)", source)
        self.assertIn("drawPlanBGauge", source)
        self.assertIn("drawMenuBarQuotaWindows(", source)
        self.assertIn("window: window.label", source)
        self.assertNotIn('drawMenuBarUsagePercentRow(window: "5h"', source)
        self.assertNotIn('drawMenuBarUsagePercentRow(window: "7d"', source)
        ...
        self.assertIn("text: quotaResetCountdown(window)", source)
        ...
        self.assertIn("quotaResetCountdown", source)
        ...
        self.assertIn("let fillInset: CGFloat = 1.0", source)
        self.assertIn("let innerTrackRect = statusPixelAlignedRect(trackRect.insetBy(dx: fillInset, dy: fillInset))", source)
        self.assertIn("let minimumVisibleFillWidth: CGFloat = 2.4", source)
        self.assertIn("max(minimumVisibleFillWidth, innerTrackRect.width * fraction)", source)
    ...
    def test_status_item_view_avoids_raster_image_scaling(self):
        ...
        self.assertIn("statusPixelAlignedRect", source.split("private func drawMenuBarQuotaWindows", 1)[1])
    ...
    def test_native_app_dropdown_keeps_detailed_codex_pro_meter_rows(self):
        ...
        self.assertIn('"\\(window.label) left', source)
        self.assertIn('"\\(window.label) resets', source)
    ...
    def test_native_app_can_download_and_install_confirmed_github_updates(self):
        ...
        self.assertIn('AGENT_LABEL="app.codexgauge.menubar"', source)
        self.assertIn("install_launch_agent", source)
        self.assertIn("<key>RunAtLoad</key>", source)
        self.assertIn('/usr/bin/open -na "$TARGET_APP"', source)
    ...
    def test_native_app_has_session_only_preferences_for_refresh_notifications_and_theme(self):
        ...
        self.assertIn("isLaunchAgentConfigured()", source)
    ...
    def test_native_app_sends_opt_in_quota_notifications(self):
        ...
        self.assertIn("quotaLowNotificationPrefix", source)
        self.assertIn("quotaRestoredNotificationPrefix", source)
        self.assertIn("liveUnavailableNotification", source)
        self.assertIn("previousQuotaLeftByWindow", source)
