...
class PublicReleaseHygieneTests(unittest.TestCase):
    ...
    def test_public_asset_generator_draws_menu_bar_strip(self):
        ...
        self.assertIn("xRadius: 1.6", script)
        self.assertIn("let innerRect = rect.insetBy(dx: 2.0, dy: 2.0)", script)
        self.assertIn("NSColor(calibratedRed: 0.43, green: 0.49, blue: 0.63, alpha: 0.94)", script)
        self.assertIn("NSColor(calibratedWhite: 0.08, alpha: 0.88)", script)
