...
class PublicReleaseHygieneTests(unittest.TestCase):
    ...
    def test_public_asset_generator_draws_menu_bar_strip(self):
        ...
        self.assertIn("let fillInset: CGFloat = 1.0", script)
        self.assertIn("let innerRect = rect.insetBy(dx: fillInset, dy: fillInset)", script)
        self.assertIn("max(4.4, innerRect.width * fraction)", script)
        ...
        self.assertIn("NSColor(deviceWhite: 0.08, alpha: 0.95)", script)
        self.assertNotIn("NSColor(calibratedWhite: 0.08, alpha: 0.07)", script)
