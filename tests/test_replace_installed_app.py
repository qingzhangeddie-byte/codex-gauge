...
class ReplaceInstalledAppScriptTests(unittest.TestCase):
    def test_replacement_script_removes_old_app_copies_new_app_and_launches_target(self):
        ...
        self.assertIn('install_launch_agent "$TARGET_APP"', script)
        ...
        self.assertIn('APP_BINARY_NAME="$APP_NAME"', build_script)
        self.assertIn('LEGACY_APP_BINARY_NAME="${APP_NAME}-bin"', build_script)
        ...
        self.assertNotIn('Contents/MacOS/$APP_NAME-bin', script)
    ...
    def test_admin_fallback_passes_paths_to_osascript_as_arguments(self):
        script = pathlib.Path("script/replace_installed_app.sh").read_text()
        fallback = script.split("replace_with_admin() {", 1)[1].split("install_user_app()", 1)[0]

        self.assertIn("/usr/bin/osascript", fallback)
        self.assertIn("on run argv", fallback)
        self.assertIn("set sourceApp to item 1 of argv", fallback)
        self.assertIn("set targetApp to item 2 of argv", fallback)
        self.assertIn("set legacyTargetApp to item 3 of argv", fallback)
        self.assertIn('"$SOURCE_APP" "$TARGET_APP" "$LEGACY_TARGET_APP"', fallback)
        self.assertNotIn('set sourceApp to "$SOURCE_APP"', fallback)
        self.assertNotIn("<<APPLESCRIPT", fallback)
    ...
    def test_main_build_script_strips_macos_metadata_before_codesign(self):
        ...
        self.assertNotIn('SSD_TEMPERATURE_HELPER="$stage_resources/ssd_temperature"', script)
        self.assertNotIn('clang "$SSD_TEMPERATURE_SOURCE"', script)
        ...
        self.assertIn('install_launch_agent "$target"', script)
        self.assertIn('install_launch_agent "$user_target"', script)
        self.assertIn("<key>RunAtLoad</key>", script)
        self.assertIn("enabled startup launch", script)
        self.assertIn("temporary non-persistent app", script)
        self.assertLess(script.index('swiftc "$BUILD_MAIN" -o "$stage_binary"'), script.index('sign_app_bundle "$stage_bundle"'))
